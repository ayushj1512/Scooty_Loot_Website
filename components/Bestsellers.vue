<template>
  ...
  <section class="py-12 bg-gradient-to-b from-gray-50 to-white">
    <div class="max-w-7xl mx-auto px-4">
      ...
      <!-- Skeleton Loader -->
      <div v-if="loadingBS" class="grid grid-cols-2 max-[429px]:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 justify-items-center">
        <div
          v-for="i in skeletonCount"
          :key="i"
          class="relative bg-gray-100 shimmer-card h-64 rounded-xl shadow p-4 w-full max-w-md overflow-hidden"
        >
...
const skeletonCount = ref(8) // default fallback

// Set shimmer count based on screen
const setShimmerByScreen = () => {
  const width = window.innerWidth
  if (width < 640) {
    skeletonCount.value = 2 * 2 // 2 rows of 2 cols
  } else if (width < 1024) {
    skeletonCount.value = 3 * 3 // 3 rows of 3 cols
  } else {
    skeletonCount.value = 4 * 4 // 4 rows of 4 cols
  }
}
...
onMounted(() => {
  setShimmerByScreen()
  fetchBestsellers()
  window.addEventListener('resize', setShimmerByScreen)
})
</script>
